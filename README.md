...
独立的 AstrBot 插件，将显式白名单中的普通命令和子命令自动包装为具有结构化参数的模型工具，支持文本、At（@ 提及）、图片和文件回复。
...
- 命令使用独立事件，结果、停止标志、会话对象与参数信息不会污染原聊天事件。含 At、图片或文件的回复使用原聊天事件的适配器接口发送，并产生正常的发送标记。命令对数据库、插件状态等产生的业务效果仍然存在。
...
- 支持声明式注册的异步命令和子命令，回复支持 `Plain`、`At`、`Image`、`File` 及其混排；不把正则监听器、纯指令组入口、多个 command 装饰器叠加的处理器包装成工具。
- 含 `At`、图片或文件的消息链会原样发送到当前会话，保留组件顺序及 @ 对象，`sent_messages` 统计发送的消息链数；其中的文本同时收集为工具输出。纯文本回复仅作为工具输出返回模型。
...
- 语音、视频、合并转发等其他组件、流式回复和需要额外流水线处理的返回值会明确报错；同一消息链存在不支持的组件时，整条拒绝发送。
